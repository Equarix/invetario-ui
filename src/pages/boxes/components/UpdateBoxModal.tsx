...
export default function UpdateBoxModal({
  isOpen,
  onOpenChange,
  box,
}: UpdateBoxModalProps) {
  ...
  useEffect(() => {
    if (box) {
      reset({
        ...
        storeId: box.storeId ? String(box.storeId) : "",
      });
    }
  }, [box, reset]);
  ...
  // Update box mutation (PUT /box/:boxId)
  const updateMutation = useMutation({
    mutationFn: async (data: UpdateBoxFormData) => {
      if (!box) return;
      const payload = {
  ...
  const onSubmit = (formData: UpdateBoxFormData) => {
    setErrorMsg(null);
        ...
        {(onClose) => (
          <form onSubmit={handleSubmit(onSubmit)}>
            <ModalHeader className="flex items-center gap-2">
              ...
              <span>Editar Caja #{box?.boxId}</span>
            </ModalHeader>
